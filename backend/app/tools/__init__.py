...
def create_tool_registry(tavily_api_key: str | None) -> ToolRegistry:
    ...
    for tool in create_local_write_tools():
        registry.register(ToolSpec(
            ...
            version="1.0.0",
            tool=tool,
            category="documents" if tool.name.startswith(("word_", "presentation_")) else "local",
            risk_level="write",
            side_effect=True,
            safe_artifact_write=tool.name.startswith(("word_", "presentation_")),
            idempotent=False,
            timeout_seconds=180.0 if tool.name.startswith(("word_", "presentation_")) else 10.0,
            allowed_agents=frozenset({"chat_agent"}),
