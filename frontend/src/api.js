...
export function sendMessage(conversationId, message, apiKey, tavilyApiKey, useRag = true, mode = "auto", embeddingApiKey = "", embeddingBaseUrl = "", attachments = []) {
  const headers = {
  ...
  return request(`/conversations/${conversationId}/messages`, {
    method: "POST",
    headers,
    body: JSON.stringify({ message, attachments, use_rag: useRag, mode }),
  });
}
